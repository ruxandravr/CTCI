...
public:
	...
	bool checkDuplicates_v1(std::string &s);
	bool checkDuplicates_v2(std::string &s);

	bool checkPermutation_v1(std::string& s1, std::string& s2);
	bool checkPermutation_v2(std::string& s1, std::string& s2);

	/*  Assume we get a URL array with the size before URLify. 
		We return a string formed from the URLified array */
	std::string URLify(std::vector<char>& url);

};

/* Returns true if a string has duplicates
   [additional data structure] */
bool ArraysAndStrings::checkDuplicates_v1(std::string &s) {
	...
	for (int i = 0; i < s.length(); ++i) {
		if (s[i] - 'a' < 0 || s[i] - 'a' > 26) {
			std::cout << "Error processing string\n";
			return false;
		}
...
/* Returns true if a string has duplicates
   [bit manipulation] */
bool ArraysAndStrings::checkDuplicates_v2(std::string &s) {
...
bool ArraysAndStrings::checkPermutation_v1(std::string &s1, std::string &s2) {
...
bool ArraysAndStrings::checkPermutation_v2(std::string &s1, std::string &s2) {
